...
3. Database config
   ...
   Edit config/database.rb to let ProxyFS connect to your database.
...
* +touch+ is not yet supported.
  ...
  SFTP offers the manipulation of +file+ +attributes+ and ProxyFS will possibly use this feature in the future.
...
* Empty files possibly won't be created.
  ...
  will <b>of course</b> create a new file <b>test.txt</b>.
...
* A file's timestamp will be different on each mirror, because ProxyFS does not mind, yet.
  ...
  Possibly, this behaviour will change in the future.
